...
execute at @e[type=minecraft:squid,name="Tente"] run execute unless blocks -129 61 290 -126 63 294 ~-2 ~ ~-2 all run execute at @e[type=minecraft:squid,name="Tente"] run summon minecraft:item ~ ~1 ~ {Item: {id: "minecraft:squid_spawn_egg", count: 1, components:{"minecraft:enchantments": {levels:{"minecraft:unbreaking": 1}, show_in_tooltip: false}, "minecraft:custom_name": '"Tente"', "minecraft:lore": ['"Monte une tente"', '"Clic droit au sol sur un terrain dégagé"']}}}
